  ...
  const analizzaAcqua = () => {
    ...
    if (cloro >= 0) {
      if (cloro < 1) {
        ...
        const cloroNecessario = (1 - cloro) * 20 * volume / 1000; // 20 g/mc per 1 ppm, convertito in kg
        correzioni.push(`Aumentare la produzione di cloro dalla cella elettrolitica o aggiungere ${Math.round(cloroNecessario * 100) / 100} kg di cloro granulare per portare a 1 ppm`);
    ...
    if (alcalinita > 0) {
      if (alcalinita < limiti.alcalinita[0]) {
        ...
        correzioni.push(`Aggiungere ${Math.round((alcalinita - limiti.alcalinita[1]) * 0.2 * volume * 100) / 100} litri di acido`);
    ...
    if (tac > 0) {
      if (tac < limiti.tac[0]) {
        ...
        correzioni.push(`Aggiungere ${Math.round((tac - limiti.tac[1]) * 0.2 * volume * 100) / 100} litri di acido`);
        problemiBagnanti.push('Difficoltà nel mantenere il pH stabile');
        problemiPiscina.push('Formazione di calcare');
      }
    }
